...
AniLinux - это десктопное приложение для просмотра аниме, построенное на Electron с интеграцией OAuth авторизации через Shikimori. Поддерживает Windows, Linux и macOS.
...
- npm
- mpv плеер:
  - **Windows**: Скачайте с https://mpv.io/installation/
  - **Linux**: `sudo pacman -S mpv` (Arch) или `sudo apt install mpv` (Debian/Ubuntu)
...
- **Windows**: `%APPDATA%\anilinux\anilinux-store.json`
- **macOS**: `~/Library/Application Support/anilinux/anilinux-store.json`
...
- **Windows**: Скачайте установщик с https://mpv.io/installation/ и добавьте mpv.exe в PATH
- **Linux**: `sudo pacman -S mpv` (Arch) или `sudo apt install mpv` (Debian/Ubuntu) или `sudo dnf install mpv` (Fedora)
- **macOS**: `brew install mpv`
